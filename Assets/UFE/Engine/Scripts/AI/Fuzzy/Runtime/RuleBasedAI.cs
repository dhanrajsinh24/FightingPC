...
public class RuleBasedAI : RandomAI{
		...
		if (self != null && opponent != null){
			if (this.aiOutput != null){
				...
				if (this.aiOutput.TryGetValue(AIReaction.JumpBackward, out weight) && this.ValidateReaction(AIReactionType.JumpBack, self, opponent)){
					...
					if (!Mathf.Approximately((float)this.inputBuffer[this.inputBuffer.Count - 1][this.horizontalAxis].axisRaw, 0f)){
	...
	protected virtual void AddValidMoves(
		...
		//-------------------------------------------------------------------------------------------------------------
		int index = 0;
		for (int i = 0; i < self.myInfo.loadedMoves.Length; ++i){
			MoveSetData move = self.myInfo.loadedMoves[i];
			...
			for (int j = 0; j < move.attackMoves.Length; ++j){
				...
				if (moveInfo != null && self.MoveSet.ValidateMoveExecution(moveInfo)){
					...
					if (ai.advancedOptions.reactionParameters.enableGaugeFilter){
						float gaugeUsage = (float)moveInfo._gaugeUsage / maxGaugePoints;
	...
	protected virtual void OnGameBegin(UFE3D.CharacterInfo player1, UFE3D.CharacterInfo player2, StageOptions stage){
		...
		if (self != null){
			...
			jumpForward.Add(new ButtonPress[]{ButtonPress.Forward});
	...
	protected virtual void RequestAIUpdate(ControlsScript self, ControlsScript opponent, float deltaTime){
		if (self != null && opponent != null){
			...
			float characterGaugeSelf = (float)self.myInfo.currentGaugePoints / (float)(self.myInfo.maxGaugePoints);
            float characterHealthSelf = (float)self.myInfo.currentLifePoints / (float)(self.myInfo.lifePoints);
			...
			if (myMoveInfo != null){
				...
				attackGaugeSelf = (float)(myMoveInfo._gaugeUsage) / (float)(self.myInfo.maxGaugePoints);
            ...
            float characterGaugeOpponent = (float)opponent.myInfo.currentGaugePoints / (float)(opponent.myInfo.maxGaugePoints);
            float characterHealthOpponent = (float)opponent.myInfo.currentLifePoints / (float)(opponent.myInfo.lifePoints);
			...
			if (opMoveInfo != null){
				...
				attackGaugeOpponent = (float)(opMoveInfo._gaugeUsage) / (float)(opponent.myInfo.maxGaugePoints);
