...
public class MoveEditorWindow : EditorWindow {
	...
	public void OnGUI(){
		...
		if (moveInfo.animMap.clip != null){
            totalFramesOriginal = (int)Mathf.Abs(Mathf.Ceil((moveInfo.fps * moveInfo.animMap.clip.length) / (float)moveInfo._animationSpeed));
			...
			EditorGUILayout.BeginVertical(rootGroupStyle);{
				...
				if (moveInfo.gaugeToggle){
					EditorGUILayout.BeginVertical(subGroupStyle);{
                        ...
                        SubGroupTitle("Self");
                        moveInfo._gaugeGainOnHit = StyledSlider("Gauge Gain on Hit (%)", (float)moveInfo._gaugeGainOnHit, EditorGUI.indentLevel, 0, 100);
                        moveInfo._gaugeGainOnMiss = StyledSlider("Gauge Gain on Cast (%)", (float)moveInfo._gaugeGainOnMiss, EditorGUI.indentLevel, 0, 100);
                        moveInfo._gaugeGainOnBlock = StyledSlider("Gauge Gain on Block (%)", (float)moveInfo._gaugeGainOnBlock, EditorGUI.indentLevel, 0, 100);
                        moveInfo._gaugeRequired = StyledSlider("Gauge Required (%)", (float)moveInfo._gaugeRequired, EditorGUI.indentLevel, 0, 100);
                        moveInfo._gaugeUsage = StyledSlider("Gauge Cost (%)", (float)moveInfo._gaugeUsage, EditorGUI.indentLevel, 0, 100);

                        EditorGUILayout.Space();
                        EditorGUIUtility.labelWidth = 200;
                        EditorGUI.BeginDisabledGroup(moveInfo.stopDrainingGauge);
                        {

                            moveInfo.startDrainingGauge = EditorGUILayout.Toggle("Start Draining", moveInfo.startDrainingGauge, toggleStyle);
                            if (moveInfo.startDrainingGauge) {
                                moveInfo.inhibitGainWhileDraining = EditorGUILayout.Toggle("- Inhibit Gauge Gain", moveInfo.inhibitGainWhileDraining, toggleStyle);
                                moveInfo._gaugeDPS = EditorGUILayout.FloatField("- Drain Per Second (%):", (float)moveInfo._gaugeDPS);
                                moveInfo._totalDrain = EditorGUILayout.FloatField("- Total Drain (%):", (float)moveInfo._totalDrain);
                                moveInfo.DCMove = (MoveInfo)EditorGUILayout.ObjectField("- Move (Drain Complete):", moveInfo.DCMove, typeof(MoveInfo), false);
                                moveInfo.DCStance = (CombatStances)EditorGUILayout.EnumPopup("- Stance (Drain Complete):", moveInfo.DCStance, enumStyle);
                            }
                        } 
                        EditorGUI.EndDisabledGroup();
                        ...
                        moveInfo._opGaugeGainOnHit = StyledSlider("Gauge Gain on Hit (%)", (float)moveInfo._opGaugeGainOnHit, EditorGUI.indentLevel, 0, 100);
                        moveInfo._opGaugeGainOnBlock = StyledSlider("Gauge Gain on Block (%)", (float)moveInfo._opGaugeGainOnBlock, EditorGUI.indentLevel, 0, 100);
                        moveInfo._opGaugeGainOnParry = StyledSlider("Gauge Gain on Parry (%)", (float)moveInfo._opGaugeGainOnParry, EditorGUI.indentLevel, 0, 100);
                        EditorGUILayout.Space();
			...
			EditorGUILayout.BeginVertical(rootGroupStyle);{
				...
				if (animationOptions){
					EditorGUILayout.BeginVertical(subGroupStyle);{
						...
						SubGroupTitle("File");
						moveInfo.animMap.clip = (AnimationClip) EditorGUILayout.ObjectField("Animation Clip:", moveInfo.animMap.clip, typeof(UnityEngine.AnimationClip), true);
						if (moveInfo.animMap.clip != null){
						...
						for (int i = 0; i < moveInfo.opponentOverride.Length; i ++){
							...
							EditorGUILayout.BeginVertical(arrayElementStyle);{
								...
								if (opTransformToggle){
									EditorGUILayout.BeginVertical(subArrayElementStyle);{
										moveInfo.opponentOverride[i]._position = FPVector.ToFPVector(EditorGUILayout.Vector3Field("Position", moveInfo.opponentOverride[i]._position.ToVector()));
							...
							EditorGUILayout.BeginVertical(arrayElementStyle);{
								...
								EditorGUI.BeginDisabledGroup(moveInfo.projectiles[i].projectilePrefab == null);{
									...
									if (moveInfo.projectiles[i].hurtBox.shape == HitBoxShape.circle){
										...
										moveInfo.projectiles[i].hurtBox._rect = new FPRect(moveInfo.projectiles[i].hurtBox.rect);
				...
				EditorGUILayout.BeginVertical(rootGroupStyle);{
					...
					if (classificationOptions){
						EditorGUILayout.BeginVertical(subGroupStyle);{
							...
							if (moveInfo._gaugeUsage < 50) gaugeUsageTemp = GaugeUsage.Quarter;
							else if (moveInfo._gaugeUsage < 75) gaugeUsageTemp = GaugeUsage.Half;
							else if (moveInfo._gaugeUsage < 100) gaugeUsageTemp = GaugeUsage.ThreeQuarters;
							else if (moveInfo._gaugeUsage >= 100) gaugeUsageTemp = GaugeUsage.All;
	...
	public void AnimationSampler(GameObject targetChar, AnimationClip animationClip, int castingFrame, bool loadHitBoxes, bool loadHurtBoxes, bool mirror, bool invertRotation){
        ...
        hitBoxesScript.UpdateMap((int)animFrame);*/
        if (loadHitBoxes){
        ...
        /*Transform[] childTransforms = targetChar.GetComponentsInChildren<Transform>();
        foreach (Transform childTransform in childTransforms) {
            Vector3 localPos = childTransform.localPosition;
            //localPos.x = 0;
            //localPos.z = 0;
            if (childTransform.name == "Root") childTransform.localPosition = localPos;
        }*/
	}
